...
### Results
**Preliminary (`--quick` smoke: h=1, 3 seeds, ~12k-hr span)** — the full run (h=1 & h=24, 5 seeds,
~26k-hr span) is in progress and this table will be replaced with its committed numbers:

| model | RMSE (°C) | MAE | skill vs persistence |
|---|---|---|---|
| Persistence | 0.970 | 0.672 | 0.0% |
| Linear (AR-X) | 0.695 | 0.491 | 28.3% |
| ESN | **0.678** | 0.477 | 30.0% |
| RFF | 0.682 | 0.481 | 29.6% |
| CHIMERA | 0.685 | 0.483 | 29.3% |

Best classical = ESN; **CHIMERA vs ESN: DM(HAC) = +2.49, p = 0.013 (CHIMERA slightly worse)**.
Reservoirs add real value over persistence/linear (~30% skill — weather has more short-term
nonlinear structure than RV), but the quantum reservoir is **competitive, not better** — the same
honest pattern as Track A.
...
## Honest bottom line (to be finalized with the full numbers)
Across Track A (volatility) and Track B (weather), at simulable scale the quantum reservoir is
*competitive and distinct* but not *better* than strong classical baselines — the negative appears
to be **domain-general**, not specific to realized volatility. The submission stays V1.
